...
class Amount(dict):
    """ This class deals with Amounts of any asset to simplify dealing with the tuple::
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a list of a ``float`` and a :class:`vinchainio.asset.Asset`
        ...
        An instance is a dictionary and comes with the following keys:
            ...
            * ``amount`` (float)
            * ``symbol`` (str)
            * ``asset`` (instance of :class:`vinchainio.asset.Asset`)
    ...
    def __init__(self, *args, amount=None, asset=None, vinchain_instance=None):
        ...
        if len(args) == 1 and isinstance(args[0], Amount):
            ...
            self["asset"] = Asset(self["symbol"], vinchain_instance=self.vinchain)

        elif (len(args) == 1 and
                isinstance(args[0], dict) and
                "amount" in args[0] and
                "asset_id" in args[0]):
            self["asset"] = Asset(args[0]["asset_id"], vinchain_instance=self.vinchain)
            ...
            self["amount"] = int(args[0]["amount"]) / 10 ** self["asset"]["precision"]
                ...
                "amount" in args[0] and
                "asset" in args[0]):
            self["asset"] = Asset(args[0]["asset"], vinchain_instance=self.vinchain)
            self["symbol"] = self["asset"]["symbol"]
            self["amount"] = int(args[0]["amount"]) / 10 ** self["asset"]["precision"]
    ...
    @property
    def asset(self):
        """ Returns the asset as instance of :class:`vinchainio.asset.Asset`
        """
        if not self["asset"]:
            self["asset"] = Asset(self["symbol"], vinchain_instance=self.vinchain)
        return self["asset"]
