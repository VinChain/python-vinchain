...
from vinchainiobase.asset_permissions import (
    asset_permissions,
    force_flag,
    test_permissions,
    todict
...
class Asset(BlockchainObject):
    ...
    def refresh(self):
        ...
        """
        asset = self.vinchain.rpc.get_asset(self.identifier)
        if not asset:
            raise AssetDoesNotExistsException(self.identifier)
        super(Asset, self).__init__(asset, vinchain_instance=self.vinchain)
        if self.full:
            if "bitasset_data_id" in asset:
                self["bitasset_data"] = self.vinchain.rpc.get_object(
                    asset["bitasset_data_id"])
            self["dynamic_asset_data"] = self.vinchain.rpc.get_object(
    ...
    def get_call_orders(self, limit=100):
        ...
        ret = self.vinchain.rpc.get_call_orders(self["id"], limit)
    ...
    def get_settle_orders(self, limit=100):
        ...
        assert self.is_bitasset
        r = list()
        ret = self.vinchain.rpc.get_settle_orders(self["id"], limit)
    ...
    def halt(self):
        ...
        return self.vinchain.finalizeOp(op, self["issuer"], "active")
    ...
    def seize(self, from_account, to_account, amount):
        """ Seize amount from an account and send to another
            ...
            :param vinchainio.account.Account from_account: From this account
            :param vinchainio.account.Account to_account: To this account
            :param vinchainio.amount.Amount amount: Amount to seize
        """
    ...
    def set_market_fee(self, percentage_fee, max_market_fee):
        """ Set trading percentage fee
            ...
            :param vinchainio.amount.Amount max_market_fee: Max Fee
