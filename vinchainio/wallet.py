...
from vinchainiobase.account import PrivateKey, GPHPrivateKey
...
class Wallet():
    """ The wallet is meant to maintain access to private keys for
        ...
        :param VinChainNodeRPC rpc: RPC connection to a VinChain node
    ...
    def getAllAccounts(self, pub):
        ...
        for id in self.getAccountsFromPublicKey(pub):
            try:
                account = Account(id)   # FIXME: self.vinchain is not available in wallet!
            except:
                continue
    ...
    def getAccount(self, pub):
        ...
        if not name:
            ...
            try:
                account = Account(name)   # FIXME: self.vinchain is not available in wallet!
            except:
                return
