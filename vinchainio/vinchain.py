...
from vinchainioapi.vinchainnoderpc import VinChainNodeRPC
from vinchainiobase import operations
from vinchainiobase.account import PublicKey
from vinchainiobase.chains import known_chains as vinchain_known_chains
...
class VinChain(object):
    """ Connect to the VinChain network.
        ...
        :param dict known_chains: Override known_chains
        ...
        * **Wallet Database**: Here, the bitshareslibs load the keys from the
          ...
          To use this mode, simply call ``VinChain()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``VinChain()``.
    ...
    def __init__(self,
        ...
        if known_chains:
            vinchain_known_chains.clear()
            for chain in known_chains:
                vinchain_known_chains[chain] = known_chains[chain]
    ...
    def connect(self,
                ...
                rpcpassword="",
                **kwargs):
        """ Connect to VinChain network (internal use only)
        """
        if not node:
            if "node" in config:
                node = config["node"]
            else:
                raise ValueError("A VinChain node needs to be provided!")
        ...
        self.rpc = VinChainNodeRPC(node, rpcuser, rpcpassword, **kwargs)
    ...
    def finalizeOp(self, ops, account, permission, **kwargs):
        """ This method obtains the required private keys if present in
            ...
            :param object append_to: This allows to provide an instance of
                ProposalsBuilder (see :func:`vinchainio.new_proposal`) or
                TransactionBuilder (see :func:`vinchainio.new_tx()`) to specify
            ...
            ... note:: This uses ``vinchainio.txbuffer`` as instance of
                :class:`vinchainio.transactionbuilder.TransactionBuilder`.
                You may want to use your own txbuffer
        """
        if "append_to" in kwargs and kwargs["append_to"]:
            if self.proposer:
                log.warn(
                    "You may not use append_to and vinchainio.proposer at "
                    "the same time. Append vinchainio.new_proposal(..) instead"
                )
    ...
    def sign(self, tx=None, wifs=[]):
        ...
        """
        if tx:
            txbuffer = TransactionBuilder(tx, vinchain_instance=self)
        else:
    ...
    def broadcast(self, tx=None):
        """ Broadcast a transaction to the VinChain network
    ...
    def newWallet(self, pwd):
        """ Create a new wallet. This method is basically only calls
            :func:`vinchainio.wallet.create`.

            :param str pwd: Password to use for the new wallet
            :raises vinchainio.exceptions.WalletExists: if there is already a
                wallet created
        """
    ...
    def transfer(self, to, amount, asset, memo="", account=None, **kwargs):
        ...
        account = Account(account, vinchain_instance=self)
        amount = Amount(amount, asset, vinchain_instance=self)
        to = Account(to, vinchain_instance=self)
    ...
    def create_account(
        ...
        """ Create new account on VinChain

            The brainkey/password can be used to recover all generated keys
            (see `vinchainiobase.account` for more details.
        ...
        try:
            Account(account_name, vinchain_instance=self)
            raise AccountExistsException
        except:
            pass

        referrer = Account(referrer, vinchain_instance=self)
        registrar = Account(registrar, vinchain_instance=self)

        " Generate new keys from password"
        from vinchainiobase.account import PasswordKey, PublicKey
        if password:
        ...
        for k in additional_owner_accounts:
            addaccount = Account(k, vinchain_instance=self)
        ...
        voting_account = Account(
            proxy_account or "proxy-to-self", vinchain_instance=self)
    ...
    def allow(
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, vinchain_instance=self)
    ...
    def approvewitness(self, witnesses, account=None, **kwargs):
        ...
        for witness in witnesses:
            witness = Witness(witness, vinchain_instance=self)
    ...
    def approvecommittee(self, committees, account=None, **kwargs):
        ...
        for committee in committees:
            committee = Committee(committee, vinchain_instance=self)
    ...
    def approveproposal(
        ...
        if not approver and not is_key:
            ...
            approver = Account(approver, vinchain_instance=self)
        else:
        ...
        for proposal_id in proposal_ids:
            proposal = Proposal(proposal_id, vinchain_instance=self)
            update_dict = {
                "fee": {"amount": 0, "asset_id": "1.3.0"},
    ...
    def approveworker(self, workers, account=None, **kwargs):
        ...
        for worker in workers:
            worker = Worker(worker, vinchain_instance=self)
    ...
    def cancel(self, orderNumbers, account=None, **kwargs):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=False, vinchain_instance=self)
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        """ Withdraw vesting balance
            ...
            :param vinchainio.amount.Amount Amount: to withdraw ("all" if not provided")
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        ...
        if not amount:
            obj = Vesting(vesting_id, vinchain_instance=self)
    ...
    def publish_price_feed(
        ...
        """ Publish a price feed for a market-pegged asset
            ...
            :param vinchainio.price.Price settlement_price: Price for settlement
            :param vinchainio.price.Price cer: Core exchange Rate (default ``settlement_price + 5%``)
        ...
        assert mcr > 100
        assert mssr > 100
        assert isinstance(settlement_price, Price), "settlement_price needs to be instance of `vinchainio.price.Price`!"
        assert cer is None or isinstance(cer, Price), "cer needs to be instance of `vinchainio.price.Price`!"
        if not account:
            if "default_account" in config:
                account = config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, vinchain_instance=self)
        asset = Asset(symbol, vinchain_instance=self, full=True)
    ...
    def reserve(self, amount, account=None, **kwargs):
        """ Reserve/Burn an amount of this shares
            ...
            :param vinchainio.amount.Amount amount: The amount to be burned.
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
    ...
    def create_worker(
        ...
        """ Reserve/Burn an amount of this shares
            ...
            **Required**
            ...
            :param vinchainio.amount.Amount daily_pay: The amount to be paid daily
            ...
            **Optional**
            ...
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        from vinchainiobase.transactions import timeformat
    ...
    def fund_fee_pool(self, symbol, amount, account=None, **kwargs):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, vinchain_instance=self)
        asset = Asset(symbol, vinchain_instance=self)
    ...
    def vindb_block_create(self, owner, block_id, block_hash, **kwargs):
        owner = Account(owner, vinchain_instance=self)

        op = operations.Vindb_block_create(**{
            'owner': owner["id"],
            'block_id': block_id,
    ...
    def invoice_create(self, contractor, customer, vin, report_uuid,
                       amount, asset, contractor_reward, records, **kwargs):
        ...
        for record in records:
            data_source = Account(record['data_source'], vinchain_instance=self)
            reward = Amount(record['reward'], asset, vinchain_instance=self)

            op_records.append({
                'data_source': data_source['id'],
                'hash': record['hash'],
                'value': record['value'] * operations.Invoice_vehicle_record.GRAPHENE_BLOCKCHAIN_PRECISION,
                'reward': {
                    'amount': int(reward),
                    'asset_id': reward.asset['id']
                }
            })
        ...
        return self.finalizeOp(op, contractor, "active", **kwargs)

    def invoice_payment_create(self, account, report_uuid, **kwargs):
        account = Account(account, vinchain_instance=self)

        op = operations.Invoice_payment_create(**{
            'from': account['id'],
            'report_uuid': report_uuid,
            'fee': {
                'amount': 0,
    ...
    def get_vindb_blocks(self, block_id, limit):
        return self.rpc.get_vindb_blocks(block_id, limit)

    def get_latest_vindb_block(self):
        return self.rpc.get_latest_vindb_block()

    def get_invoice_by_report_uuid(self, report_uuid):
        return self.rpc.get_invoice_by_report_uuid(report_uuid)

    def get_message(self, *args, **kwargs):
        return Message(vinchain_instance=self, *args, **kwargs)
