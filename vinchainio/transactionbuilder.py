...
from vinchainiobase.objects import Operation
from vinchainiobase.account import PrivateKey, PublicKey
from vinchainiobase.signedtransactions import Signed_Transaction
from vinchainiobase import transactions, operations
...
class ProposalBuilder:
    """ Proposal Builder allows us to construct an independent Proposal
        ...
        :param int proposal_review: Number of seconds for review of the
            proposal
        :param vinchainio.transactionbuilder.TransactionBuilder: Specify
            your own instance of transaction builder (optional)
        :param vinchainio.vinchain.VinChain vinchain_instance: VinChain
            instance
    """
    def __init__(
        self,
...
class TransactionBuilder(dict):
    ...
    def appendSigner(self, account, permission):
        ...
        if self.vinchain.wallet.locked():
            raise WalletLocked()
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[perm]["account_auths"]:
                    auth_account = Account(
                        authority[0], vinchain_instance=self.vinchain)
        ...
        if account not in self.signing_accounts:
            ...
            if isinstance(account, PublicKey):
                self.wifs.add(
                    self.vinchain.wallet.getPrivateKeyForPublicKey(
                        str(account)
                    )
                )
            else:
    ...
    def constructTx(self):
        ...
        ops = transactions.addRequiredFees(self.vinchain.rpc, ops)
        expiration = transactions.formatTimeFromNow(
            self.expiration or self.vinchain.expiration
        )
    ...
    def sign(self):
        ...
        if self.vinchain.proposer:
            proposer = Account(
                self.vinchain.proposer,
                vinchain_instance=self.vinchain)
            self.wifs = set()
        ...
        # presented wrongly!
        if self.vinchain.rpc:
            operations.default_prefix = (
                self.vinchain.rpc.chain_params["prefix"])
        ...
        signedtx.sign(self.wifs, chain=self.vinchain.rpc.chain_params)
    ...
    def verify_authority(self):
        ...
        """
        try:
            if not self.vinchain.rpc.verify_authority(self.json()):
                raise InsufficientAuthorityError
        except Exception as e:
            raise e

    def broadcast(self):
        """ Broadcast a transaction to the vinchainio network
        ...
        # Broadcast
        try:
            if self.vinchain.blocking:
                ret = self.vinchain.rpc.broadcast_transaction_synchronous(
                    ret, api="network_broadcast")
                ret.update(**ret["trx"])
            else:
                self.vinchain.rpc.broadcast_transaction(
                    ret, api="network_broadcast")
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        self.constructTx()
        self["blockchain"] = self.vinchain.rpc.chain_params
    ...
    def appendMissingSignatures(self):
        ...
        for pub in missing_signatures:
            wif = self.vinchain.wallet.getPrivateKeyForPublicKey(pub)
            if wif:
                self.appendWif(wif)
