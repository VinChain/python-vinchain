...
class Price(dict):
    """ This class deals with all sorts of prices of any pair of assets to
        ...
        each being an instance of :class:`vinchainio.amount.Amount`. The
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`vinchainio.asset.Asset`
            ...
            * ``args`` being a list of ``[quote, base]`` both being instances of :class:`vinchainio.amount.Amount`
            * ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
            * ``base`` and ``quote`` being instances of :class:`vinchainio.asset.Amount`
        ...
        * ``Price(1.0, "USD/GOLD")``
        ...
        .. code-block:: python
            ...
            >>> from vinchainio.price import Price
    ...
    def __init__(
        ...
        *args,
        ...
        if (len(args) == 1 and isinstance(args[0], str) and not base and not quote):
            ...
            self["quote"] = Amount(amount=frac.denominator, asset=quote, vinchain_instance=self.vinchain)
            self["base"] = Amount(amount=frac.numerator, asset=base, vinchain_instance=self.vinchain)
            ...
            if args[0]["base"]["asset_id"] == base_id:
                self["base"] = Amount(args[0]["base"], vinchain_instance=self.vinchain)
                self["quote"] = Amount(args[0]["quote"], vinchain_instance=self.vinchain)
            else:
                self["quote"] = Amount(args[0]["base"], vinchain_instance=self.vinchain)
                self["base"] = Amount(args[0]["quote"], vinchain_instance=self.vinchain)
            ...
            if args[0]["receives"]["asset_id"] == base_asset["id"]:
                ...
                self["base"] = Amount(args[0]["receives"], vinchain_instance=self.vinchain)
                self["quote"] = Amount(args[0]["pays"], vinchain_instance=self.vinchain)
                self["type"] = "sell"
            else:
                # buy order
                self["base"] = Amount(args[0]["pays"], vinchain_instance=self.vinchain)
                self["quote"] = Amount(args[0]["receives"], vinchain_instance=self.vinchain)
                self["type"] = "buy"
    ...
    def market(self):
        """ Open the corresponding market
            ...
            :returns: Instance of :class:`vinchainio.market.Market` for the
...
class Order(Price):
    """ This class inherits :class:`vinchainio.price.Price` but has the ``base``
    ...
    def __init__(self, *args, vinchain_instance=None, **kwargs):
        ...
        if (
            ...
            super(Order, self).__init__(
                Amount(args[0]["min_to_receive"], vinchain_instance=self.vinchain),
                Amount(args[0]["amount_to_sell"], vinchain_instance=self.vinchain),
            )
            self["id"] = args[0].get("id")
