...
class Market(dict):
    """ This class allows to easily access Markets on the blockchain for trading, etc.
        ...
        :param vinchainio.asset.Asset base: Base asset
        :param vinchainio.asset.Asset quote: Quote asset
        ...
        * ``base`` and ``quote`` are valid assets (according to :class:`vinchainio.asset.Asset`)
        ...
        * ``base-quote`` separated with ``-``
        ...
        .. note:: Throughout this library, the ``quote`` symbol will be
                  ...
                  :func:`vinchainio.market.Market.sell` or
                  :func:`vinchainio.market.Market.buy`, you will sell/buy **only
    ...
    def ticker(self):
        ...
        if "bitasset_data_id" in self["quote"]:
            bitasset = self.vinchain.rpc.get_object(self["quote"]["bitasset_data_id"])
            ...
            bitasset = self.vinchain.rpc.get_object(self["base"]["bitasset_data_id"])
            backing_asset_id = bitasset["options"]["short_backing_asset"]
        ...
        ticker = self.vinchain.rpc.get_ticker(
            self["base"]["id"],
            self["quote"]["id"],
        )
        data["baseVolume"] = Amount(ticker["base_volume"], self["base"], vinchain_instance=self.vinchain)
        data["quoteVolume"] = Amount(ticker["quote_volume"], self["quote"], vinchain_instance=self.vinchain)
    ...
    def volume24h(self):
        ...
        """
        volume = self.vinchain.rpc.get_24_volume(
            self["base"]["id"],
            self["quote"]["id"],
        )
        return {
            self["base"]["symbol"]: Amount(volume["base_volume"], self["base"], vinchain_instance=self.vinchain),
            self["quote"]["symbol"]: Amount(volume["quote_volume"], self["quote"], vinchain_instance=self.vinchain)
        }
    ...
    def orderbook(self, limit=25):
        """ Returns the order book for a given market. You may also
            ...
            .. note:: Each bid is an instance of
                class:`vinchainio.price.Order` and thus carries the keys
                ``base``, ``quote`` and ``price``. From those you can
                obtain the actual amounts for sale

        """
        orders = self.vinchain.rpc.get_order_book(
            self["base"]["id"],
            self["quote"]["id"],
            limit
        )
        asks = list(map(lambda x: Order(
            Amount(x["quote"], self["quote"], vinchain_instance=self.vinchain),
            Amount(x["base"], self["base"], vinchain_instance=self.vinchain),
            vinchain_instance=self.vinchain
        ), orders["asks"]))
    ...
    def trades(self, limit=25, start=None, stop=None):
        ...
        orders = self.vinchain.rpc.get_trade_history(
            self["base"]["symbol"],
            self["quote"]["symbol"],
            formatTime(stop),
            formatTime(start),
        ...
        return list(map(
            lambda x: FilledOrder(
                x,
                quote=Amount(x["amount"], self["quote"], vinchain_instance=self.vinchain),
                base=Amount(float(x["amount"]) * float(x["price"]), self["base"], vinchain_instance=self.vinchain),
                vinchain_instance=self.vinchain
            ), orders
        ))
    ...
    def accounttrades(self, account=None, limit=25):
        ...
        """
        if not account:
            if "default_account" in self.vinchain.config:
                account = self.vinchain.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, vinchain_instance=self.vinchain)

        filled = self.vinchain.rpc.get_fill_order_history(
            self["base"]["id"],
            self["quote"]["id"],
            2 * limit,
            api="history"
    ...
    def accountopenorders(self, account=None):
        """ Returns open Orders
            ...
            :param vinchainio.account.Account account: Account name or instance of Account to show orders for in this market
        """
        if not account:
            if "default_account" in self.vinchain.config:
                account = self.vinchain.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=True, vinchain_instance=self.vinchain)
    ...
    def buy(
        ...
        """
        if not expiration:
            expiration = self.vinchain.config["order-expiration"]
        if not account:
            if "default_account" in self.vinchain.config:
                account = self.vinchain.config["default_account"]
        if not account:
        ...
        if isinstance(amount, Amount):
            amount = Amount(amount, vinchain_instance=self.vinchain)
            assert(amount["asset"]["symbol"] == self["quote"]["symbol"]), \
                "Price: {} does not match amount: {}".format(
                    str(price), str(amount))
        else:
            amount = Amount(amount, self["quote"]["symbol"], vinchain_instance=self.vinchain)
        ...
        if returnOrderId:
            ...
            prevblocking = self.vinchain.blocking
            self.vinchain.blocking = returnOrderId

        tx = self.vinchain.finalizeOp(order, account["name"], "active")

        if returnOrderId:
            tx["orderid"] = tx["operation_results"][0][1]
            self.vinchain.blocking = prevblocking
    ...
    def cancel(self, orderNumber, account=None):
        ...
        """
        return self.vinchain.cancel(orderNumber, account=account)
