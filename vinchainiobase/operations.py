...
class Vindb_block_create(GrapheneObject):
...
class Invoice_vehicle_record(GrapheneObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isArgsThisClass(self, args):
            ...
            super().__init__(OrderedDict([
                ('data_source', ObjectId(kwargs["data_source"], "account")),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isArgsThisClass(self, args):
            ...
            super().__init__(OrderedDict([
                ...
                ('records', Array([Invoice_vehicle_record(r) for r in kwargs["records"]])),
                ('fee', Asset(kwargs["fee"])),
            ]))


class Invoice_payment_create(GrapheneObject):
    def __init__(self, *args, **kwargs):
        # Allow for overwrite of prefix
        if isArgsThisClass(self, args):
            self.data = args[0].data
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            super().__init__(OrderedDict([
                ...
                ('active_approvals_to_add', Array([ObjectId(o, "account") for o in kwargs["active_approvals_to_add"]])),
                ('active_approvals_to_remove', Array([
                    ObjectId(o, "account") for o in kwargs["active_approvals_to_remove"]
                ])),
                ('owner_approvals_to_add', Array([ObjectId(o, "account") for o in kwargs["owner_approvals_to_add"]])),
                ('owner_approvals_to_remove', Array([
                    ObjectId(o, "account") for o in kwargs["owner_approvals_to_remove"]
                ])),
                ('key_approvals_to_add', Array([PublicKey(o) for o in kwargs["key_approvals_to_add"]])),
                ('key_approvals_to_remove', Array([PublicKey(o) for o in kwargs["key_approvals_to_remove"]])),
                ('extensions', Set([])),
            ]))
